...
for (let a of [0, 0.001, 0.002, 0.003, 0.004, 0.005]) {
    let ok = 0;
    for (const c of cases) if (JSON.stringify(simWindow(c.input, sl, a)) === JSON.stringify(c.exp)) ok++;
    console.log(`alpha=${a.toFixed(3)} Global Score: ${ok}/100`);
}
