...
function simulate(race, fb) {
    ...
    for (let lap = 1; lap <= total; lap++) {
        // Linear fuel burn: car gets faster as lap increases
        const fuelEffect = -fb * (lap - 1);
        for (const c of cars) {
            c.age++;
            ...
            const lapTime = base * (1 + p.offset[ti] + wearEffect) 
                   + fuelEffect
...
console.log('Sweeping Fuel Burn (fb)...');
for (let fb = 0.0; fb <= 0.1; fb += 0.01) {
    let ok = 0;
    for (const c of cases) {
        if (JSON.stringify(simulate(c.input, fb)) === JSON.stringify(c.expected)) ok++;
    }
    console.log(`fb ${fb.toFixed(2)}: ${ok}/100`);
}
