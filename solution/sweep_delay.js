...
function simulate(race, df) {
    ...
    for (let i = 1; i <= 20; i++) {
        ...
        cars.push({ id: s.driver_id, grid: i, tire: s.starting_tire, age: 0, time: (i-1)*df, stops: s.pit_stops || [], si: 0 });
    }
    for (let lap = 1; lap <= total; lap++) {
        ...
        pitting.sort((a,b) => a.time - b.time);
        pitting.forEach((c) => {
            c.time += pit;
...
console.log('Testing Start Delay Factor sweep...');
for (let df = 0; df <= 0.01; df += 0.001) {
    let ok = 0;
    for (const c of cases) {
        if (JSON.stringify(simulate(c.input, df)) === JSON.stringify(c.expected)) ok++;
    }
    console.log(`df ${df.toFixed(3)}: ${ok}/100`);
}
