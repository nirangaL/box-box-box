...
function simulate(race, qPen) {
    ...
    for (let lap = 1; lap <= total; lap++) {
        for (const c of cars) {
            ...
            c.time += base * (1 + best.offset[ti] + wearEffect)
                     + (c.age === 1 ? best.freshBonus[ti] : 0)
                     + (c.si > 0 && c.age === 1 ? best.pitExitPenalty : 0);
        }
...
console.log('Testing specific qPen values...');
[0, 0.5, 1.0, 1.5, 2.0].forEach(q => {
    let ok = 0;
    for (const c of cases) {
        if (JSON.stringify(simulate(c.input, q)) === JSON.stringify(c.expected)) ok++;
    }
    console.log(`qPen ${q}: ${ok}/100`);
});
